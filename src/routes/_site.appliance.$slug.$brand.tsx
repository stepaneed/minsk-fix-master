...
export const Route = createFileRoute("/_site/appliance/$slug/$brand")({
  loader: async ({ params }) => {
    const [{ data: service }, { data: brand }] = await Promise.all([
      supabase.from("service_types").select("id,slug,title,description").eq("slug", params.slug).eq("is_active", true).maybeSingle(),
      supabase.from("brands").select("id,slug,title,logo_url,logo_scale").eq("slug", params.brand).eq("is_active", true).maybeSingle(),
    ]);
