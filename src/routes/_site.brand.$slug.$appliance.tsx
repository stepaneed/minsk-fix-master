...
export const Route = createFileRoute("/_site/brand/$slug/$appliance")({
  loader: async ({ params }) => {
    const [{ data: brand }, { data: service }] = await Promise.all([
      supabase.from("brands").select("id,slug,title,logo_url,logo_scale").eq("slug", params.slug).eq("is_active", true).maybeSingle(),
      supabase.from("service_types").select("id,slug,title,description").eq("slug", params.appliance).eq("is_active", true).maybeSingle(),
    ]);
